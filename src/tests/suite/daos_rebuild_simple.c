		       ...
		       daos_size_t size)
{
	...
	for (i = 0; i < 20; i++) {
		...
		rc = dfs_open(dfs_mt, dir, filename, S_IFREG | S_IWUSR | S_IRUSR,
			      O_RDONLY | O_EXCL, OC_RP_2G1, 1048576, NULL, &obj);
		assert_int_equal(rc, 0);
		  ...
		  daos_size_t size, uint32_t flags)
{
	...
	for (i = 0; i < 20; i++) {
		sprintf(filename, "rebuild_file_%d", i);
		rc = dfs_open(dfs_mt, dir, filename, S_IFREG | S_IWUSR | S_IRUSR,
			      flags, OC_RP_2G1, 1048576, NULL, &obj);
		assert_int_equal(rc, 0);
...
static const struct CMUnitTest rebuild_tests[] = {
	...
	{"REBUILD23: rebuild with dfs inflight append",
	 rebuild_with_dfs_inflight_append, rebuild_small_sub_rf1_setup, test_teardown},
	{"REBUILD24: rebuild with dfs inflight punch",
