...
#define dtx_evict_lid(cont, dae)							\
	do {										\
		...
		lrua_evictx(cont->vc_dtx_array,						\
			    (DAE_LID(dae) & ~DTX_LID_SOLO_FLAG) - DTX_LID_RESERVED,	\
			    DAE_EPOCH(dae));						\
	} while (0)
			   ...
			   daos_epoch_t epoch, uint32_t intent, uint32_t type, bool retry)
{
	...
	if (entry & DTX_LID_SOLO_FLAG) {
		if ((intent == DAOS_INTENT_DEFAULT || intent == DAOS_INTENT_IGNORE_NONCOMMITTED) &&
		    cont->vc_solo_dtx_epoch < epoch) {
			...
			uint32_t type, uint32_t *tx_id)
{
	...
	if ((dth->dth_need_validation || unlikely(dth->dth_aborted)) && !dth->dth_verified) {
